<script setup>
  ...
  let openTasks = computed(() => props.myTasks.filter((tasks) => !tasks.complete))
  let completedTasks = computed(() => props.myTasks.filter((tasks) => tasks.complete))
...
<template>
  <div>
    ...
    <TaskAdd id="addTask" @add-task="useAddTask(props.tasks, $event , props.options)" />
